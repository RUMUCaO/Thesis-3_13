        ...
        if DEVICE == "cuda" and PREFER_FAST_SINGLE_GPU:
            try:
                ...
                if not (FALLBACK_TO_AUTO_ON_OOM and is_oom):
                    raise
                print("Fast single-GPU load OOM, falling back to device_map='auto'.")
...
def describe_scene(frames, scene_id, start, end):
    ...
    prompt = (
        "Analyze this scene and return ONLY a JSON object with this schema:\n"
        "{\n"
        "  \"entities\": [{\"name\": str, \"type\": str, \"attributes\": [str]}],\n"
        "  \"actions\": [str],\n"
        "  \"events\": [{\"event\": str, \"participants\": [str], \"time_hint\": str}],\n"
