...
def _parse_scenes_rule_based(text):
    ...
    scene_counter = 0
    pending_character = None          # colon‑cue or all‑caps cue awaiting next line
    metadata_skipped = False          # skip preamble until first real content
    last_was_action = False           # used to detect scene‑level action blocks
    ...
    # ------------------------------------------------------------
    # Helpers (simplified versions of the universal‑parser logic)
    # ------------------------------------------------------------
    def is_meta_line(l):
        """Metadata lines that should be skipped entirely."""
        meta_patterns = [
            r'^\d+\s+Contributors', r'^Lyrics', r'^Source:', r'^https?://',
            r'^Formatted for', r'^SCENE HEADINGS', r'^CHARACTER NAMES',
            r'^TRANSITIONS', r'^\d{4}\s*$', r'^Genius', r'^Movie Scripts',
            r'^Script$', r'^Movie Script$', r'^\d+\s*$'
        ]
        return any(re.match(p, l) for p in meta_patterns)

    def is_scene_boundary(l):
        """Return True if this line should trigger a new scene."""
        # 1. Standard scene heading
        if SCENE_HEADING_PATTERN.match(l):
            return True
        # 2. Full‑uppercase line that looks like a scene description
        #    (longer than a character name, containing keywords)
        if l.isupper() and len(l) >= 10:
            if any(k in l.upper() for k in ['ESTABLISHING', 'MONTAGE', 'SERIES OF SHOTS']):
                return True
        return False

    def is_character_cue(l):
        """Detect any style of character cue."""
        s = l.strip()
        if not s: return False
        # Colon style: "Sid Arbuck:" / "JENNY:"
        if s.endswith(':') and len(s) < 50 and not s.startswith(('http','www')):
            return True
        # All‑caps short line (1‑4 words), not a scene heading
        if s.isupper() and 1 <= len(s.split()) <= 4 and len(s) < 50:
            if not is_scene_boundary(s):
                return True
        # Mixed case + colon, e.g. "Harry (V.O.):"
        if re.fullmatch(r"[A-Z][A-Za-z ]+(\([A-Za-z. ]+\))?:", s):
            return True
        # Uppercase with parenthetical tag, e.g. "HOLLY (V.O.)"
        if re.fullmatch(r"[A-Z ]+(\([A-Za-z. ]+\))", s):
            return True
        return False

    def is_parenthetical(l):
        """Parenthetical / bracket direction."""
        s = l.strip()
        return (s.startswith('(') and s.endswith(')')) or (s.startswith('[') and s.endswith(']'))

    def is_dash_dialogue(l):
        """Line starting with '- ' is a dialogue line."""
        return l.startswith('- ')
    ...
    for i, line in enumerate(lines):
        ...
        # Skip metadata preamble
        if not metadata_skipped:
            if is_meta_line(stripped) or stripped == '':
                continue
            metadata_skipped = True

        # Blank line → reset speaker, could be a scene break if a long action block has passed
        if not stripped:
            current_speaker = None
        ...
        # If we've been in an action block for several lines and now hit a
        # character cue or dash dialogue, start a new scene.
        if current_scene["raw_text_span"] and last_was_action:
            if is_character_cue(stripped) or is_dash_dialogue(stripped):
                flush_scene()
                last_was_action = False

        current_scene["raw_text_span"].append(stripped)

        # ---------- Dialogue triggered by dash ----------
        if is_dash_dialogue(stripped):
            current_speaker = None
            pending_character = None
            _maybe_add_dialogue(current_scene, "UNKNOWN", stripped[2:].strip())
            last_was_action = False
            continue

        # ---------- Character cue ----------
        if is_character_cue(stripped):
            # Is there dialogue immediately after colon?
            if ':' in stripped:
                rest = re.sub(r'^.*?:\s*', '', stripped)
                name = re.sub(r':$', '', stripped).strip()
                name = re.sub(r'\s*\(.*?\)\s*', '', name).strip()
                if rest:
                    # Cue + dialogue on the same line
                    current_speaker = name
                    _append_unique(current_scene["characters"], current_speaker)
                    _maybe_add_dialogue(current_scene, current_speaker, rest)
                    pending_character = None
                    last_was_action = False
                    continue
            # Standalone character cue – wait for next line
            name = re.sub(r':$', '', stripped).strip()
            name = re.sub(r'\s*\(.*?\)\s*', '', name).strip()
        ...
        if is_parenthetical(stripped):
            if pending_character:
                current_speaker = pending_character
                _append_unique(current_scene["characters"], current_speaker)
        ...
        # ---------- Regular line (dialogue or action) ----------
        if pending_character:
            current_speaker = pending_character
            _append_unique(current_scene["characters"], current_speaker)
            _maybe_add_dialogue(current_scene, current_speaker, stripped)
        ...
        if current_speaker:
            # still within the same character's speech
            _maybe_add_dialogue(current_scene, current_speaker, stripped)
            last_was_action = False
        else:
            # Action / narration
...
def parse_script(text, normalize_characters=False, min_scenes=2, min_dialogues=3):
    characters = set()
    ...
    char_stats = defaultdict(lambda: {"scenes": set(), "dialogue_count": 0})
    
    for scene in scenes:

        for speaker in scene.get("characters", []):
            char_stats[speaker]["scenes"].add(scene["scene_id"])

        for block in scene.get("dialogue_blocks", []):
            speaker = block.get("speaker")
            if speaker:
                char_stats[speaker]["scenes"].add(scene["scene_id"])
                char_stats[speaker]["dialogue_count"] += 1
    ...
    for name, stats in char_stats.items():
        scene_cnt = len(stats["scenes"])
        dialogue_cnt = stats["dialogue_count"]
        if scene_cnt >= min_scenes and dialogue_cnt >= min_dialogues:
            important_characters.add(name)

    for scene in scenes:
        scene["characters"] = [c for c in scene.get("characters", []) if c in important_characters]

    characters = important_characters
...
def main():
    ...
    structured = parse_script(cleaned, normalize_characters=RUN_NORMALIZE_CHARACTERS, min_scenes=4, min_dialogues=4)
