...
class TemporalMemoryTransformer(torch.nn.Module):
    def __init__(self, input_dim=512, hidden_dim=512, num_layers=2):
        ...
        encoder_layer = torch.nn.TransformerEncoderLayer(
            d_model=input_dim, nhead=8, dim_feedforward=hidden_dim*2, dropout=0.1, batch_first=False
        )
    ...
    def update_memory(self, scene_id, shot_embedding, keyframes=None):
        ...
        self.memory_meta.append({"scene_id": scene_id, "keyframes": keyframes or []})
...
def preprocess_frames_batch(frames):
    ...
    frame_rgbs = [cv2.resize(f, (VISION_INPUT_SIZE, VISION_INPUT_SIZE)) for f in frame_rgbs]
...
def select_sampled_frames(frames, fps_value, target_fps=SAMPLE_FPS, max_frames=MAX_VISION_FRAMES_PER_SHOT):
    ...
    stride = max(1, int(round((fps_value or 1) / target_fps)))
...
def extract_keyframes(frames, num_frames=MAX_FRAMES_PER_SHOT):
    if not frames: return []
    if len(frames) <= num_frames: keyframes = frames
    else:
        idx = np.linspace(0, len(frames)-1, num_frames, dtype=int)
        keyframes = [frames[i] for i in idx]
    pil_frames = [Image.fromarray(cv2.cvtColor(f, cv2.COLOR_BGR2RGB)) for f in keyframes]
    return pil_frames
...
def llm_should_analyze(scene_id, shot_embedding, prev_shot_embedding):
    ...
    if prev_shot_embedding is not None:
        sim = torch.nn.functional.cosine_similarity(shot_embedding.unsqueeze(0), prev_shot_embedding.unsqueeze(0), dim=1).item()
