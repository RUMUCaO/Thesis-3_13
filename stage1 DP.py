...
def _parse_script_universal(script_path: str) -> List[Dict[str, Any]]:
    """
    Universal Hollywood screenplay parser.

    Automatically handles:
    - Standard UPPERCASE character cues
    - Character cues with colons (Sid Arbuck: / JENNY:)
    - Dash-prefixed dialogue (- I'm fine.)
    - Narration / action blocks
    - Front matter metadata (title, source, formatting notes)
    - Page-number glue (Page X of Y)
    - Parenthetical & bracket stage directions
    """
    with open(script_path, "r", encoding="utf-8", errors="ignore") as f:
        raw_text = f.read()
    ...
    # ---- Preprocessing: strip page-number glue ----
    cleaned_lines = []
    for line in lines:
        # Remove "Page X of Y" but keep the remaining text
        line = re.sub(r'Page\s+\d+\s+of\s+\d+', '', line, flags=re.IGNORECASE)
        cleaned_lines.append(line.strip())

    # ---- State machine ----
    segments: List[Dict[str, Any]] = []
    current_type = None          # 'action' or 'dialogue'
    current_lines = []
    pending_character = None     # previous line was a short uppercase cue awaiting next line
    ...
    def is_meta(line: str) -> bool:
        """Detect obvious metadata / header / footer lines."""
        meta_patterns = [
    ...
    def is_scene_heading(line: str) -> bool:
        """Scene heading: INT./EXT. or full-uppercase with scene keywords."""
        up = line.upper()
        if re.match(r'^(?:INT|EXT|I/E|INT/EXT|EST)\b', up):
            return True
        if up == line and len(line) <= 80:
            keywords = ['DAY', 'NIGHT', 'CONTINUOUS', 'LATER', 'MONTAGE',
                        'FADE IN', 'FADE OUT', 'CUT TO']
            if any(k in up for k in keywords) and len(up.split()) <= 12:
                return True
        return False

    def is_character_cue(line: str) -> bool:
        """Character cue: short uppercase line or short line ending with a colon."""
        s = line.strip()
        if not s:
            return False
        # Colon-terminated: "Sid Arbuck:", "JENNY:", "Harry (V.O.):"
        if s.endswith(':') and len(s) < 50:
            if not s.startswith(('http', 'www')):
                return True
        # Full uppercase, 1-4 words, reasonable length
        if s.isupper() and 1 <= len(s.split()) <= 4 and len(s) < 50:
            if not is_scene_heading(s):
                return True
        # Mixed case + colon, e.g. "Harry (V.O.):"
        if re.fullmatch(r"[A-Z][A-Za-z ]+(\([A-Za-z. ]+\))?:", s):
            return True
        # Uppercase with a parenthetical tag, e.g. "HOLLY (V.O.)"
        if re.fullmatch(r"[A-Z ]+(\([A-Za-z. ]+\))", s):
            return True
        return False

    def is_parenthetical(line: str) -> bool:
        """Parenthetical stage directions (including square brackets)."""
        s = line.strip()
    ...
    for line in cleaned_lines:
        ...
        # Skip metadata until the first real content line
        if not content_started:
            if is_meta(line) or line.startswith('* * *'):
                continue
            # Determine if this line looks like real content
            if (is_scene_heading(line) or
                is_character_cue(line) or
                line.startswith('- ') or          # dash-prefixed dialogue
                (line[0].isupper() and len(line) > 5 and not line.isupper())):
                content_started = True
            else:
                continue  # still in preamble

        # ----- Case 1: Dash-prefixed dialogue -----
        if line.startswith('- '):
            flush()
            current_type = 'dialogue'
            current_lines = [line[2:].strip()]   # strip leading "- "
            pending_character = None
            continue

        # ----- Case 2: Scene heading -----
        if is_scene_heading(line):
            flush()
            current_type = 'action'
            current_lines = [line]
        ...
        # ----- Case 3: Character cue -----
        if is_character_cue(line):
            flush()
            # Extract character name (remove colon and parenthetical tags)
            name = re.sub(r':$', '', line).strip()
            name = re.sub(r'\s*\(.*?\)\s*', '', name).strip()
            # If the cue line contains dialogue after the colon, use it directly
            ...
            if rest:
                ...
                # Standalone character name – dialogue follows on next line(s)
                pending_character = name
                current_type = 'dialogue'   # mark that we expect dialogue
            continue

        # ----- Case 4: Parenthetical / bracket direction -----
        if is_parenthetical(line):
            # Belongs to the current or upcoming dialogue
            if current_type == 'dialogue' or pending_character is not None:
                if pending_character:
                    # Character cue was immediately above; start dialogue with this note
                    flush()
                    current_type = 'dialogue'
                    current_lines = [line]
                    pending_character = None
        ...
        # ----- Case 5: Regular line (dialogue or narration) -----
        if pending_character:
            # Previous line was a character name; this line is their speech
            flush()
            current_type = 'dialogue'
            current_lines = [line]
            pending_character = None
        ...
        # Generic line: append to current paragraph
        if current_type is None:
            current_type = 'action'
            current_lines = [line]
        else:
...
def parse_script(script_path: str) -> List[Dict[str, Any]]:
    ...
    if lower.endswith(".pdf"):
        return _parse_script_universal(script_path)
...
if __name__ == "__main__":
    ...
    main(
        video="THE7YI.mp4",
        script="14_The_Seven_Year_Itch.pdf",
        transcription="THE7YI.srt",
        out="output.json",
